...
- **Prompt versions:** `gen-v1` produced Week 1 (Fall 2026). `gen-v2` (2026-09-08) adds three rules from that batch's curation: options in parallel form and similar length (the correct option was the longest in 28/30 gen-v1 candidates), stems that ask a direct question with a decision as its answer (no "which reasoning best matches…" and no yes/no option sets — rejected as ambiguous), and cross-source items when natural (textbook framework applied to the week's reading or news item). The generator also warns when the correct option is the longest in more than a third of a batch.
- **Generation model:** not pinned (decided 2026-09-07). The model is a per-batch choice recorded in the candidates file and in EVAL-LOG next to `promptVersion`, so accept rates can be compared across models as well as prompts. The script defaults to Claude Fable 5.1 (`PAL_MODEL` overrides) with structured outputs (JSON schema) — over-generates 3× target with mandatory per-question source citations. Week 1 (Fall 2026) is generated in an interactive Claude Code session on Fable 5.1 rather than through the script — subscription-billed, no API key — writing the same candidates file so curation and the eval log are unchanged.
...
- **Sets + per-set checkpoints (2026-08-30):** weeks group into **sets**, each closing at an in-class test — set 1 = weeks 1–5 before Test 1 (Oct 6), set 2 = weeks 6–12 before the final (Dec 1). Students hand in one completion log per set instead of a screenshot per week; 79 students × 2 submissions is a gradebook that fits in an evening. `src/assets/js/sets.js` is the single source of truth (weeks, checkpoint names, dates) — it ships to the browser and is imported by the Eleventy data file `src/_data/sets.js`, the tests, and `scripts/verify.js`; a second course instance edits that one file. Home page renders set headings with a per-set tally and **locked placeholder rows** for weeks not yet released, so the shape of the semester is visible from week 1. Weekly publish cadence unchanged: each week's set still deploys after that week's class. New Nunjucks filters `findWeek` / `setOf` (Nunjucks can't bind a variable inside a nested loop).
  *Amended 2026-08-31:* the final moved to finals week (Dec 15–21; slot TBA), so set 2's heading reads DUE FINALS WEEK via `dueLabel` in `sets.js` and the verifier's after-checkpoint check anchors to Dec 21 until the date is assigned.

...
- **Students** open a link — no account needed. Weekly sets of ~10 questions with immediate per-distractor feedback and self-explanation prompts. Missed questions come due again after 1, 3, then 7 days (SM-2-lite; ts-fsrs is the upgrade path). Progress lives in localStorage only, with self-serve backup/restore and a screenshot-friendly completion log on the `/systems/` page. The site is an installable PWA built for subway commutes: the full app shell precaches on first visit, sets keep working offline, and an interrupted quiz resumes where it left off.

...
- `npm run generate <week> [target]` — Groundwork candidate generation (local only, needs Claude API auth; writes `content/questions/week-NN.candidates.yaml`). Alternative, used for Weeks 1–2 of Fall 2026: generate in an interactive Claude Code session (subscription, no key) — reuse the SYSTEM prompt from `scripts/generate.js` (gen-v3 since 2026-09-11: parallel option lengths, direct-question stems, name the source and spell out acronyms, real-claim distractors, proportional coverage, cross-source items when natural), author the batch as structured JSON (`{type, stem, correct:{text,feedback}, distractors:[…×3], answerPos, selfExplainPrompt, sourceDoc, sourceLoc}`), then `npm run build-candidates <week> <file.json> [--model id]` writes the candidates file in the exact shape the API path produces and prints batch stats and lint warnings. Slide decks: `pdftotext` misses image-only slides — view those PDF pages directly before generating
- `npm run diff-decisions <a.json> <b.json> [--merge out.json]` — compare two curation reviews of one batch (the curator's export and an independent fresh-context review) and merge them for `curate`. Weeks 1–2 used both reviews; keep doing it
